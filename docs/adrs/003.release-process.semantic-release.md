...
`@semantic-release/npm` with `npmPublish: false` writes the bumped version to `package.json` in CI but does not publish. The actual publish is done by `clean-publish` in a separate workflow step, which strips `scripts` and `devDependencies` before calling `npm publish`. `@semantic-release/git` and `@semantic-release/changelog` intentionally omitted (tag-only).

`clean-publish` is configured via the `clean-publish` field in `package.json`:
...
- semantic-release writes the bumped version to `package.json`, then `clean-publish` strips `scripts` and `devDependencies` before publishing
- `NPM_TOKEN` — must be set as a GitHub Actions secret (Settings → Secrets → Actions). Generate from npmjs.com → Access Tokens → Automation token.
