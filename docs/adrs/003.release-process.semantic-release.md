...
wtty targets developers who already have Node.js or Bun installed. Distribution via `npx webtty` is the natural fit — no binary downloads needed. A release means publishing to npm and creating a GitHub Release with release notes. The version should be derived automatically from what changed, not set manually.

The cfx repo solved the same versioning problem with semantic-release (ADR 004): conventional commit prefixes in PR/MR titles drive version bumps post-merge, git tags are the sole version record, no release commits. That pattern maps directly to wtty, with `@semantic-release/npm` added since webtty publishes to the public npm registry.
