...
`@semantic-release/npm` publishes to npm atomically within semantic-release's publish phase — before `@semantic-release/github` creates the tag and GitHub Release. This ensures a failed npm publish prevents the tag from being created (no partial releases). `prepublishOnly` in `package.json` strips `scripts` and `devDependencies` before the package is uploaded. `@semantic-release/git` and `@semantic-release/changelog` intentionally omitted (tag-only).

`prepublishOnly` in `package.json` (runs automatically before publish):

```json
"prepublishOnly": "bun scripts/clean-pkg-scripts.ts"
```
