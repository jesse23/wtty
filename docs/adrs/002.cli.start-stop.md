...
webtty currently has one entry point: `src/server.ts`, run directly via `bun run src/server.ts` or `tsx src/server.ts`. There is no CLI, no daemon mode, and no way to stop the server other than `Ctrl-C` in the terminal that started it.
