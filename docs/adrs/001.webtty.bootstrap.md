...
The first slice of webtty is a direct port of the `ghostty-web` demo. The demo already proves the full round-trip — browser renders a terminal, user types, a real PTY responds — so rather than building from scratch, we take that working implementation and make it the foundation of webtty. Everything else (config, multi-session, CLI) is deferred.
...
**HTTP API (bootstrap slice — single hardcoded session):**

| Method | Path | Description |
|--------|------|-------------|
| `GET` | `/` | Serves terminal HTML |
| `GET` | `/dist/*` | Serves ghostty-web JS + WASM assets |
| `GET` | `/ws` | WebSocket upgrade — spawns PTY, streams I/O |
| `POST` | `/api/server/stop` | Graceful shutdown (kills PTY, closes WSS, exits) |

Multi-session REST API (`/api/sessions`) is deferred to the session management slice ([ADR 004](004.webtty.session-api.md)).
